...
local_cascade_path = r'C:\Users\kunal\OneDrive\Desktop\p8\haarcascade_frontalface_default.xml'
...
def capture_faces(name):
    ...
    while True:
        ...
        if not capturing_started:
            ...
            cv2.putText(display_frame, f"Get ready! Starting in {countdown}...", 
        ...
        cv2.putText(display_frame, progress_text, (20, 30), 
        ...
        if len(faces) > 0:
            ...
            # No face detected
            cv2.putText(display_frame, "No face detected", (180, 200), 
        ...
        if capture_start_time and time.time() - capture_start_time > capture_timeout:
            print(f"Timeout: No face detected within {capture_timeout} seconds, aborting capture.")
            video.release()
    ...
    if len(faces_data) < required_samples:
        print(f"Warning: Only captured {len(faces_data)} samples out of required {required_samples}")
        ...
        if not os.path.exists('data/faces_data.pkl'):
            ...
            try:
                ...
                if faces.size == 0:
                    ...
                    print(f"Dimension mismatch: existing={faces.shape[1]}, new={faces_data.shape[1]}")
                    ...
                    print(f"Successfully appended new face data, new shape: {faces.shape}")
            except Exception as e:
                print(f"Error loading existing face data: {str(e)}. Creating new dataset.")
                faces = faces_data
            with open('data/faces_data.pkl', 'wb') as f:
                pickle.dump(faces, f)
    ...
    try:
        ...
        try:
            ...
            if faces_data.shape[0] != len(names):
                print(f"Warning: Mismatch between faces_data.shape[0] ({faces_data.shape[0]}) and len(names) ({len(names)})")
        ...
        attendance_dir = r"C:\Users\kunal\OneDrive\Desktop\p8\Attendance"
        os.makedirs(attendance_dir, exist_ok=True)

        # Instead of using a fixed recognition threshold, we'll use confidence level
        min_confidence_for_attendance = 70  # Minimum confidence percentage to record attendance

        attendance_list = []  # List to store multiple attendance records
        attendance_recorded = {}  # Dictionary to track when attendance was last recorded
        ...
        while True:
            ...
            cv2.putText(frame, "Position your face in front of the camera", (20, 25),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
            cv2.putText(frame, "Window will close automatically after attendance is recorded", (20, 50),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 1)
            cv2.putText(frame, "Press 'q' to quit without recording, 'm' for manual attendance", (20, 75),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 1)
            ...
            for (x, y, w, h) in faces:
                ...
                try:
                    ...
                    norm_img = cv2.normalize(flattened_img, None, 0, 255, cv2.NORM_MINMAX)
                except Exception as e:
                ...
                try:
                    ...
                    distances, indices = knn.kneighbors(norm_img, n_neighbors=n_neighbors)
                    ...
                    print(f"Predicted: {prediction}, Confidence: {confidence:.0f}%, Votes: {votes}")
                    ...
                    if confidence < 40:  # Very low confidence
                        ...
                        if confidence >= min_confidence_for_attendance:
                            ...
                            if prediction not in attendance_recorded or (current_time - attendance_recorded[prediction]) > 60:
                                ts = time.time()
                                ...
                                cv2.rectangle(frame, (0, 0), (640, 100), (0, 150, 0), -1)
                                cv2.putText(frame, f"Attendance recorded for {prediction}", (20, 40),
                                           cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
                                cv2.putText(frame, "Saving and closing in 2 seconds...", (20, 80),
                                           cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 1)
                                cv2.imshow("Face Recognition", frame)
                                cv2.waitKey(2000)  # Show confirmation for 2 seconds
                                
                                # Save attendance immediately
                                save_attendance_now(attendance_list, attendance_dir, COL_NAMES, class_id)
            ...
            if key == ord('o'):  # Press 'o' to mark attendance
                ...
                for (x, y, w, h) in faces:
                    ...
                    try:
                        ...
                        cv2.putText(frame, f"Attendance recorded for {prediction}", (20, 30),
                                   cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
                        cv2.imshow("Face Recognition", frame)
...
def recognize():
    ...
    return render_template('recognize.html', teacher_logged_in=teacher_logged_in, classes=classes)
...
def show_attendance():
    ...
    file_path = os.path.join(r"C:\Users\kunal\OneDrive\Desktop\p8\Attendance", f"Attendance_{date}.csv")
    if os.path.isfile(file_path):
        with open(file_path, 'r') as csvfile:
            ...
            db_record_strings = [f"{r[0]}-{r[1]}-{r[2]}" for r in attendance_records[1:]] if len(attendance_records) > 1 else []
    ...
    return render_template('attendance.html', attendance_data=attendance_records, has_records=has_records)
...
def teacher_register():
    if request.method == 'POST':
        ...
        conn = get_db_connection()
        try:
            conn.execute('INSERT INTO teachers (username, password_hash, email, full_name) VALUES (?, ?, ?, ?)',
...
def manage_classes():
    ...
    if request.method == 'POST':
        ...
        conn = get_db_connection()
        conn.execute('INSERT INTO classes (teacher_id, name, description) VALUES (?, ?, ?)',
...
def view_class(class_id):
    ...
    class_info = conn.execute('SELECT * FROM classes WHERE id = ? AND teacher_id = ?', 
                             (class_id, session['teacher_id'])).fetchone()
...
def add_student_to_class(class_id):
    ...
    try:
        ...
        # Removed face registration restriction to allow manual additions
        # Add to class
        conn.execute(
...
def class_student_attendance(class_id):
    ...
    students = conn.execute('SELECT * FROM class_students WHERE class_id = ?', 
                           (class_id,)).fetchall()
    ...
    for student_name in student_names:
        ...
        '''
        student_records = conn.execute(query, (class_id, teacher_id, student_name)).fetchall()
    ...
    attendance_summary = []
    for student_name in student_names:
        present_count = sum(1 for date in sorted_dates if date in attendance_records[student_name])
...
def filter_class_student_attendance(class_id):
    ...
    if not class_info:
        conn.close()
        return jsonify({'success': False, 'message': 'Class not found or access denied'}), 403
    ...
    result = {
        ...
        'attendance_records': {student: {date: dict(attendance_records[student][date]) 
...
def store_attendance_in_db(student_name, date, time_str, class_id=None):
    ...
    try:
        ...
        if 'teacher_id' in session:
            ...
            if class_id is None and student_name:
                ...
                class_result = conn.execute(
                    ...
                    (student_name, teacher_id)).fetchone()
            ...
            teacher_id = 1
            print(f"No teacher logged in, using default teacher ID 1 for {student_name}")
        ...
        teacher_check = conn.execute('SELECT COUNT(*) as count FROM teachers').fetchone()
        ...
        if teacher_check['count'] == 0 and teacher_id == 1:
            try:
                conn.execute(
                    ...
                    (1, 'default', 'default_hash', 'default@example.com', 'Default Teacher')
                )
                conn.commit()
                print("Created default teacher for attendance records")
            except sqlite3.IntegrityError as e:
                print(f"Error creating default teacher: {str(e)}, but continuing anyway")
        
        # Debug the SQL command we're about to execute
        print(f"Executing INSERT with values: ({student_name}, {class_id}, {teacher_id}, {date}, {time_str}, {status})")
        ...
        if verify:
            print(f"✓ Successfully stored attendance record #{verify['id']} for {student_name} in database")
            return True
        else:
